...
class PosController extends Controller
{
    public function __construct() {
        // Staff Permission Checkget_order_summary
    ...
    public function admin_index()
    {
        ...
        return view('pos.index', compact('customers','routes'));
    }
    ...
    public function get_order_summary(Request $request){
         $routes = CustomerRoute::orderBy('id', 'asc')->get();
        return view('pos.order_summary', compact('routes'));
    ...
    public function order_store(Request $request){
        ...
        if(Session::has('pos.cart') && count(Session::get('pos.cart')) > 0){
           ...
           // $shipping_info = Session::get('pos.shipping_info');
            ...
            //USE TIME CONDITION
            $timecode=date("H",time());
            if($timecode>=14 && $timecode<=18)
            {
                //
                $order->fiscal_flag=0;
            }
            ...
            return array('success' => 0, 'message' => translate("No items in the cart"));
        }
